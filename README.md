...
| `POST /api/auth/login` | 로그인 → `{access_token, refresh_token}`. 이후 요청은 `Authorization: Bearer <access_token>` |
...
| `GET /api/agent/versions` | 사용 가능한 에이전트 버전 목록 (프론트 셀렉터 소스) |
...
| `GET /api/sessions/{id}/recommendations/{version}/export` | 확정 추천안을 다운로드용 JSON으로 내보내기 (FR-17) |
| `GET /api/admin/llm-usage/stats?days=&group_by=` | LLM 사용량 집계(총계+component/model/user별) — 모니터링·시각화 소스 (로그인 필수) |
| `GET /api/admin/audit-logs?limit=&method=&status_code=&user_id=` | 감사 로그 조회 (변경 요청 이력, 로그인 필수) |
...
> 대화 누적 게이지: `done.data.usage_gauge`(`intake_tokens`·`limit_tokens`·`ratio`·`compact_recommended`) — 턴 첫 LLM(intake) 호출의 프롬프트 토큰 기준. `compact_recommended`면 프론트가 대화 압축을 유도한다(임계 `TURN_GAUGE_LIMIT_TOKENS`, RPA-83).
> 흐름도 = `Recommendation` 트리(steps→actions→children). 수정은 UPDATE가 아니라 새 버전 INSERT라 undo·이력이 자연히 나온다.

- 에러 응답은 `{"detail": {"code": "...", "message": "사용자용 한글 메시지"}}` 형식. 스키마 검증 실패(`INVALID_RECOMMENDATION`)는 어느 필드가 왜 틀렸는지 `errors`를 덧붙인다 — `[{"field": "steps.0.actions.0.order", "reason": "Field required"}]` (최대 10건, 원문 반향 방지로 입력값은 미포함, RPA-166)
