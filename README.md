...
| `POST /api/sessions` | 빈 세션 생성 (문서 없이 멀티턴 챗 시작용) |
| `POST /api/sessions/{id}/recommend` | 최신 분석으로 추천안(흐름도) 생성·저장 (FR-09~12) — **SSE 스트림** |
...
| `POST /api/sessions/{id}/turn` | 에이전트 단일 진입점 — 한 메시지로 분석/질문/흐름도 수정, `operation="compact"`면 대화 압축 (반환 type으로 저장 분기) — **SSE 스트림** |

> 전체 흐름: `POST /documents`(저장) → `/parse`(SSE) → `/sessions/{id}/analyze`(SSE) → `/sessions/{id}/recommend`(SSE, 흐름도 생성·저장) → 프론트가 트리를 블록으로 렌더·편집 → `POST /sessions/{id}/recommendations`(편집본 새 버전). 자연어는 `POST /documents/text`로 바로 분석 단계로.
> 대화형(FR-13~16): `POST /sessions/{id}/turn` 하나로 분석·질문·흐름도 수정을 처리한다 — 백엔드가 세션에서 full context(solution/history/analysis/recommendation/parsed_doc)를 조립해 에이전트에 넘기고, 에이전트가 `solution`으로 그래프를 골라 intent를 판단한다. 반환 `type`(answer/analysis/recommendation)으로 백엔드가 저장을 분기한다.
