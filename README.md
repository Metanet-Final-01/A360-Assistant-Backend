...
> 업로드 흐름: `POST /documents`(빠른 저장) → `POST /documents/{id}/parse`(SSE 파싱) → `POST /sessions/{id}/analyze`(SSE 분석). 자연어는 `POST /documents/text`로 바로 분석 단계로.

- 에러 응답은 `{"detail": {"code": "...", "message": "사용자용 한글 메시지"}}` 형식
- 지원 형식: **PDF·PPTX·DOCX**. 표는 `{"type":"table","rows":[...]}`로 구조화 추출(PPTX·DOCX는 셀 단위, PDF는 pdfplumber)
