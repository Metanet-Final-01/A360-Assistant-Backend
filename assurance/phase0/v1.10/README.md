...
재검증 전 구현·정책 변경을 먼저 커밋한다. 검증 성공 후 갱신되는 추적 대상
`assurance/reference/rpa179/evidence/verification.json`과 `verification.sha256`도 별도 커밋해야
작업 트리가 다시 깨끗해지며, 그 뒤에만 같은 명령을 재실행할 수 있다.

```powershell
$run = [Guid]::NewGuid().ToString('N').Substring(0, 8)
python -B assurance/reference/rpa179/verify.py `
  --work-root "assurance/reference/rpa179/.work/m-$run" `
