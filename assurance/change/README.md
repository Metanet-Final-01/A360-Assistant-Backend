...
1. PR base SHA의 `requirements.txt`·`requirements-dev.txt` exact pin을 신뢰 기준 allowlist로 사용한다.
2. 새 패키지나 버전 변경은 `approved_additions`에 정확한 버전과 승인 참조가 없으면 `deny`다.
3. 현재 `policy_decision_state=decision_needed`이며 license 목록과 `high` 임계치는 Observe 측정용 제안값이지 사람 승인 정책이 아니다.
...
6. 검사기는 패키지를 설치하지 않고 네트워크 fallback도 하지 않는다.
7. 패키지 제거는 HEAD 전체의 Python import를 다시 조사한다. 참조가 남으면 `deny`, 안정적인 import-root
   매핑으로 제거를 증명하면 해당 version·취약점·license 검사는 적용하지 않는다.

CI는 PR의 요구사항을 설치하기 전에 표준 라이브러리만으로 이 작업을 실행한다. 따라서 깨끗한 runner에 없는 외부 패키지의
실제 import를 증명할 수 없으면 보수적으로 `deny` 또는 `unassured`가 된다. 결정론적 정상 fixture는 test-only adapter로
검사 로직을 검증한다. 향후 보호된 사전 구축 runner와 검토된 offline snapshot은 `RPA-183` 승격 준비에서 다룬다.
