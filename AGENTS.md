...
2. **브랜치 이름**: `<type>/<Jira키>-<영문-요약>` (예: `feat/RPA-12-rag-summary-chunking`)
   ...
   - **Atlassian MCP 도구(`mcp__atlassian__*`)가 사용 가능하면 → 직접 처리한다**:
     ...
     이슈 생성 후 Jira Automation이 GitHub 미러 이슈를 자동 생성하므로 잠시 기다렸다가
     `gh issue list --search "RPA-N in:title"`로 미러 번호를 찾아 PR 본문의 `Closes #번호`에 쓴다.
     **GitHub 이슈를 직접 만들지 않는다** (미러와 중복됨).
...
5. **PR**: 제목은 커밋 컨벤션과 동일 형식. 머지는 merge commit 방식이라 **개별 커밋 메시지가 dev 히스토리에 그대로 남는다** — 커밋 하나하나 컨벤션을 지킨다.
   PR의 영역 라벨(`백엔드`/`RAG`/`DB` 등)은 labeler 워크플로가 변경 파일 기준으로 자동 부여한다 — **수동으로 붙이지 않는다.**
...
| 폴더 | 담당 | AI 작업 가능 여부 |
|---|---|---|
| `app/` (agent 제외), `app/rag/` | 백엔드·DB·RAG | ✅ 주 작업 영역 |
...
- RAG 수집 파이프라인: `app/rag/` (Fluid Topics API 기반, `sources/`+`build/`+`store/`+`retrieval/`+`migrations/` 하위 폴더, 상세는 해당 폴더 README)
