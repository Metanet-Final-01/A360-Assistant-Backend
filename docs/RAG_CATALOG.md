...
| 액션 | 설명 | 파라미터 | 반환 |
|---|---|---|---|
| **워크시트를 데이터 테이블로 가져오기** (`getWorksheetAsDataTable`) | 지정된 워크시트에서 데이터 테이블을 생성합니다. 이 작업은 xlsx, xls, xlsb, xlsm 및 csv 파일에서 작동합니다. | 워크시트 이름 입력:(RADIO*: 활성 워크시트/특정 워크시트), 시트에 헤더 포함(CHECKBOX), [[GetWorksheetAsDataTable.readOption.label]](RADIO*: [[GetWorksheetAsDataTable.readOption.4.1.label]]/[[GetWorksheetAsDataTable.readOption.4.2.label]]), 세션 이름(SESSION*) | TABLE |
...
| **열 읽기** (`ReadExcelColumn`) | 열에서 값을 읽습니다. 이 작업은 xlsx, xls, xlsb 및 xlsm 파일에서 작동합니다. | 셀 옵션(RADIO*: 활성 셀에서/특정 셀에서), 전체 열 읽기(CHECKBOX*), [[ReadExcelColumn.readOption.label]](RADIO*: [[ReadExcelColumn.readOption.4.1.label]]/[[ReadExcelColumn.readOption.4.2.label]]), 세션 이름(SESSION*) | LIST |
| **행 읽기** (`readExcelRow`) | 행에서 값을 읽습니다. 이 작업은 xlsx, xls, xlsb 및 xlsm 파일에서 작동합니다. | 셀 옵션(RADIO*: 활성 셀에서/특정 셀에서), 전체 행 읽기(CHECKBOX*), [[ReadExcelRow.readOption.label]](RADIO*: [[ReadExcelRow.readOption.4.1.label]]/[[ReadExcelRow.readOption.4.2.label]]), 세션 이름(SESSION*) | LIST |
...
| **여러 셀 가져오기** (`GetMultipleCells`) | Excel 워크시트에서 여러 셀의 값을 검색합니다. 이 작업은 xlsx, xlx, xlsb, xlsm 및 csv 파일에서 작동합니다. | 반환할 셀 범위 선택(SELECT*: 모든 행/특정 행/셀 범위), [[GetMultipleCell.readOption.label]](RADIO*: [[GetMultipleCell.readOption.3.1.label]]/[[GetMultipleCell.readOption.3.2.label]]), 세션 이름(SESSION*) | TABLE |
...
| **단일 셀 가져오기** (`GetSingleCell`) | Excel 워크시트에서 단일 셀의 값을 검색합니다. 이 작업은 xlsx, xlx, xlsb, xlsm 및 csv 파일에서 작동합니다. | 셀 옵션(RADIO*: 활성 셀/특정 셀), [[GetSingleCell.readOption.label]](RADIO*: [[GetSingleCell.readOption.3.1.label]]/[[GetSingleCell.readOption.3.2.label]]), 세션 이름(SESSION*) | STRING |
...
| **[[CreateSession.label]]** (`createsession`) | [[CreateSession.description]] | [[CreateSession.sessionName.label]](TEXT*), [[CreateSession.consumerKey.label]](CREDENTIAL*), [[CreateSession.consumerSecret.label]](CREDENTIAL*), [[CreateSession.accessToken.label]](CREDENTIAL*), [[CreateSession.tokenSecret.label]](CREDENTIAL*) | STRING |
...
| **[[FilesinFolder.label]]** (`FilesinFolder`) | [[FilesinFolder.description]] | [[FilesinFolder.inputFolder.label]](FILE*), [[FilesinFolder.sortType.label]](SELECT*: By Date - Desc/By Date - Asc/By Name - Desc/By Name - Asc/By Size - Asc/By Size - Desc) | LIST |
...
| **함수 실행** (`RunCSharpDLL_V1`) | 매개변수를 활용해 특정 DLL 함수를 실행합니다 | [[RunCSharpDLL.runFunction.reflection.label]](DESKTOPOPERATIONBUTTON), DLL 세션(TEXT*), 네임스페이스를 입력합니다(TEXT*), 클래스 이름을 입력합니다(TEXT*), 실행할 함수의 이름을 입력합니다(TEXT*), 입력 매개변수(ENTRYLIST: 매개변수 이름/매개변수 유형/매개변수 값) | ANY |
...
| **실행** (`runTask`) | 선택한 TaskBot을 실행합니다. | 실행할 TaskBot(TASKBOT*), [[RunTask.repeatOption.label]](SELECT: 반복하지 않음/N회 반복/반복 기간/[[RunTask.repeatOption.REPEAT_UNTIL_STOPPED.label]]), 사이 시간(CHECKBOX), 에러 발생 시 다음 반복 지속(CHECKBOX) | DICTIONARY |
...
| 액션 | 설명 | 파라미터 | 반환 |
|---|---|---|---|
| **[[LoanApproval.label]]** (`loanapproval`) | [[LoanApproval.description]] | [[LoanApproval.model.label]](FILE*), [[LoanApproval.gender.label]](TEXT*), [[LoanApproval.married.label]](TEXT*), [[LoanApproval.dependents.label]](NUMBER*), [[LoanApproval.education.label]](TEXT*), [[LoanApproval.self_employed.label]](TEXT*), [[LoanApproval.applicant_income.label]](NUMBER*), [[LoanApproval.coapplicant_income.label]](NUMBER*), [[LoanApproval.loan_amount.label]](NUMBER*), [[Loan… | DICTIONARY |
