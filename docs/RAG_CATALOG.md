...
| 액션 | 설명 | 파라미터 | 반환 |
|---|---|---|---|
| **워크시트를 데이터 테이블로 가져오기** (`getWorksheetAsDataTable`) | 지정된 워크시트에서 데이터 테이블을 생성합니다. 이 작업은 xlsx, xls, xlsb, xlsm 및 csv 파일에서 작동합니다. | 워크시트 이름 입력:(RADIO*), 시트에 헤더 포함(CHECKBOX), Read option(RADIO*), 세션 이름(SESSION*) | TABLE |
...
| **열 읽기** (`ReadExcelColumn`) | 열에서 값을 읽습니다. 이 작업은 xlsx, xls, xlsb 및 xlsm 파일에서 작동합니다. | 셀 옵션(RADIO*), 전체 열 읽기(CHECKBOX*), Read option(RADIO*), 세션 이름(SESSION*) | LIST |
| **행 읽기** (`readExcelRow`) | 행에서 값을 읽습니다. 이 작업은 xlsx, xls, xlsb 및 xlsm 파일에서 작동합니다. | 셀 옵션(RADIO*), 전체 행 읽기(CHECKBOX*), Read option(RADIO*), 세션 이름(SESSION*) | LIST |
...
| **여러 셀 가져오기** (`GetMultipleCells`) | Excel 워크시트에서 여러 셀의 값을 검색합니다. 이 작업은 xlsx, xlx, xlsb, xlsm 및 csv 파일에서 작동합니다. | 반환할 셀 범위 선택(SELECT*), Read option(RADIO*), 세션 이름(SESSION*) | TABLE |
...
| **단일 셀 가져오기** (`GetSingleCell`) | Excel 워크시트에서 단일 셀의 값을 검색합니다. 이 작업은 xlsx, xlx, xlsb, xlsm 및 csv 파일에서 작동합니다. | 셀 옵션(RADIO*), Read option(RADIO*), 세션 이름(SESSION*) | STRING |
...
| **Files in Folder** (`FilesinFolder`) | Returns an ordered list of all files (including hidden files) in a directory by customizable sort order | Folder Path to Evaluate(FILE*), File sorting method(SELECT*) | LIST |
...
| **함수 실행** (`RunCSharpDLL_V1`) | 매개변수를 활용해 특정 DLL 함수를 실행합니다 | Get DLL details(DESKTOPOPERATIONBUTTON), DLL 세션(TEXT*), 네임스페이스를 입력합니다(TEXT*), 클래스 이름을 입력합니다(TEXT*), 실행할 함수의 이름을 입력합니다(TEXT*), 입력 매개변수(ENTRYLIST) | ANY |
...
| **실행** (`runTask`) | 선택한 TaskBot을 실행합니다. | 실행할 TaskBot(TASKBOT*), Repetition(SELECT), 사이 시간(CHECKBOX), 에러 발생 시 다음 반복 지속(CHECKBOX) | DICTIONARY |
...
| 액션 | 설명 | 파라미터 | 반환 |
|---|---|---|---|
| **Analyze Loan** (`loanapproval`) | Loan Approval score provided by pre-trained ML model | Loan Approval ML Model(FILE*), Gender(TEXT*), Marital Status(TEXT*), Number of Dependents(NUMBER*), College Education(TEXT*), Employer(TEXT*), Applicant Monthly Income(NUMBER*), Co-Applicant Monthly Income(NUMBER*), T… | DICTIONARY |
