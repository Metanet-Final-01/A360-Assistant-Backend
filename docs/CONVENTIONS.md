...
- [ ] 🔴 **가드는 동작이 읽는 걸 읽는가 (대리 지표 금지)** — 검사·격리·가드를 추가/수정했다면: **동작을 결정하는 값**(env가 아니라 `engine`, git이 아니라 디스크, 문서가 아니라 실행 경로)을 가드가 **직접** 읽는가? 대리 지표면 반드시 갈린다. 직접 못 읽으면 **둘이 갈리는 시나리오를 하나 만들어 보고**, 만들어지면 버그다.
  - 🔴 **변이 테스트(mutation testing)는 `scripts/mutation_test.py`로 한다** (손으로 하지 말 것):
    ...
    **결함을 심었는데 테스트가 통과하면 "좋다"가 아니라 "증명 실패"다.** 그 판정을 사람에게
    ...
    못 했는데 "통과했네" / 겨냥한 지점이 아닌 곳을 변형 / CRLF 불일치로 **변형이 안 심겼는데**
    결과를 믿음). 스크립트는 통과를 **에러(rc=1)로 강제**하고 원본을 항상 복원한다.
  - ⚠️ **`cmd | head`·`| tail`은 종료코드를 삼킨다** — `$?`는 파이프 마지막 명령의 것이다.
    검사 결과를 파이프로 보면 **실패를 성공으로 읽는다**(실측 2회). `> log; rc=$?` 로 받을 것.
  - `_x`를 monkeypatch한 테스트는 **`_x` 자신이 옳은지 아무것도 증명하지 않는다** → 진짜 파일·진짜 `.env`로 도는 테스트를 하나는 둔다.
  - <sub>근거: RPA-186 한 PR에서 같은 버그 6회 — 셸env↔`.env`(load_dotenv), engine(import)↔`_url()`(호출), 규약↔자동실행, 브랜치이름↔머지여부, HEAD↔작업트리, `git status`↔디스크(무시된 파일).</sub>
