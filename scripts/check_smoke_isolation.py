...
def main() -> int:
    ...
    if host not in _LOCAL_HOSTS:
        # 원인을 정확히 짚는다 — 앱 DB가 원격인 경로가 둘이라 사유가 틀리면 엉뚱한 곳을 고친다.
        cause = ("APP_DATABASE_URL이 설정돼 있습니다" if os.getenv("APP_DATABASE_URL", "").strip()
                 else f"DATABASE_HOST={host}")
        print(f"SHARED:관측은 앱 DB로 폴백하는데 그 앱 DB가 원격입니다 ({cause})")
        return 0
