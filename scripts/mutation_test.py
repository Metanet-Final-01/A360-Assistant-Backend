"""변이 테스트(mutation testing)를 기계적으로 수행한다 (RPA-189).

코드에 **결함을 일부러 심고**(변이) 그 테스트가 깨지는지 본다. 테스트가 통과하는 이유는 둘이다:
  ① 코드가 옳다  ② **테스트가 아무것도 안 본다**
변이를 심었는데도 통과하면 그 테스트는 장식이다 — 결함을 잡지 못한다.
...
이 스크립트가 존재하는 이유 — 2026-07-16 하루에 **변이 테스트를 3번 헛다리 짚었다**:

  1. `이미 최신` early-return에 걸려 **게이트에 도달조차 안 했는데** "통과했네"로 읽음
  2. `subject` 대입만 try 밖으로 옮기고 **빌더는 안에 남겨** 겨냥한 지점을 안 건드림
  3. 치환 문자열이 CRLF와 안 맞아 **변이가 아예 안 심겼는데** 결과를 그대로 믿음

셋 다 원인이 같다: **변이 후 테스트가 통과하면 "좋다"가 아니라 "검증 실패"인데** 그걸 반대로
읽었다. 그래서 이 스크립트는 그 판정을 사람에게 맡기지 않는다 — **통과 = 에러**로 강제한다.

⚠️ 변이 지점을 **실제 결함 패턴에 맞춰 고를 것.** 검출하기 쉬운 지점(예: 함수가 무조건 True를
반환하게)만 건드리면 통과해도 의미가 없다 — 실제로 RPA-200에서 그렇게 해놓고 "검증됐다"고
보고했다가, 진짜 버그 패턴(조회 후 갱신)으로 되돌리니 테스트 29개가 전부 통과했다.

종료코드: 0 = 검증됨(변이 시 테스트 실패) / 1 = 검증 실패(변이가 무효거나 테스트가 장식)
...
_TEST_TIMEOUT_SEC = int(os.getenv("MUTATION_TEST_TIMEOUT_SEC", "300"))
...
def _run_test(test: str) -> tuple[str, str]:
    ...
    ⚠️ 타임아웃을 "fail"로 뭉개면 안 된다 (#263 2차): 결함 삽입 후 pytest가 **멈춘** 것을
       "테스트가 결함을 잡았다"로 오판한다. 멈춤은 검증이 아니라 판정 불가다 — 3상태로 구분한다.
    """
    try:
        r = subprocess.run(
...
def main() -> int:
    ...
    if out_after == "timeout":
        ...
        print(f"🔴 **판정 불가** — 변형 후 테스트가 {_TEST_TIMEOUT_SEC}초 안에 끝나지 않았다.\n"
              f"   멈춤 ≠ 검증. 다른 결함으로 다시 시도할 것. (원본은 복원됨)", file=sys.stderr)
        return 1
    ...
    print(f"✅ 변이 테스트 통과 — 결함을 심으니 테스트가 깨진다.\n"
          f"   변형 전: {sum_before}\n"
          f"   변형 후: {sum_after}\n"
