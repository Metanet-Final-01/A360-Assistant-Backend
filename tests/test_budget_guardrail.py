...
def test_observability_query_failure_does_not_break_business_turn(monkeypatch):
    """관측 DB 쿼리 장애도 예산을 fail-open하고 업무 요청을 500으로 만들지 않는다."""
    ...
    class _Session:
        ...
        def execute(self, _statement):
            raise SQLAlchemyError("observability unavailable")

    monkeypatch.setattr(obs, "observability_sessionmaker", lambda: _Session)
    verdict = budget.check_budget(budget.subject_of(None, SID), now=NOW)
    assert verdict.exceeded is False
