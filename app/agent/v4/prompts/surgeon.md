...
- **세션 핸들이 다른 패키지로 넘어감(R17)** → 그 액션을 **핸들을 연 패키지의 액션으로**
  update 한다. **[패키지 교체 후보] 블록에 자리마다 고를 수 있는 이름이 나열돼 있으니 거기서
  그대로 복사한다.** ⚠ 지금 액션 이름을 옮겨 갈 패키지에 그대로 붙이면
  (`Microsoft 365 Excel/Save workbook action in Excel advanced package` 같은) 없는 표기가 되어
  연산이 통째로 무시된다 — 실측에서 3라운드 연속 이렇게 버려졌다.
