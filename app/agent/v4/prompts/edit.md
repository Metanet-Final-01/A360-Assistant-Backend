...
- set_spec: 이 흐름도가 충족해야 할 **요구 목록**을 바꾼다. id는 [현재 흐름도가 충족해야 할 요구]의 값.
    { "op":"set_spec", "remove_req_ids":["req-3"] }                       ← 그 업무가 더는 필요 없을 때
    { "op":"set_spec", "requirements":[{"text":"실패 시 담당자에게 알린다","priority":"must"}] }  ← 요구 추가
    { "op":"set_spec", "requirements":[{"req_id":"req-2","text":"…수정된 문장…"}] }               ← 기존 요구 수정
    remove_req_ids/requirements는 목록 **전체 교체가 아니라** 지정한 항목만 지우고/더한다.
...
9. **"이 단계 빼주세요"는 두 가지를 함께 한다.** 어떤 업무 자체가 필요 없어졌다는 요청이면
   ① 해당 액션을 remove 하고, ② 그 업무에 대응하는 요구를 set_spec의 remove_req_ids로 지운다.
   액션만 지우면 요구가 남아, 검수가 "요구를 실현하는 액션이 없다"고 판정해 **방금 지운 액션을
   도로 넣는다** — 사용자 입장에서는 삭제가 먹지 않은 것으로 보인다.
   반대로 "지금은 빼되 요구 자체는 살려두라"(나중에 다시 넣을 것)면 요구는 그대로 두고, 검수에서
   미실현으로 잡힐 수 있다는 점을 answer에서 알린다.
   ⚠ 없앨 요구를 특정하지 못하면 지우지 말고 answer에서 어느 요구인지 되물어라 — 요구를 잘못
   지우면 그 업무가 이후 어떤 검수에도 안 걸린다.
10. **모르는 호환성을 지어내지 않는다.** 어떤 액션이 특정 OS에서 동작하는지는 카탈로그가
