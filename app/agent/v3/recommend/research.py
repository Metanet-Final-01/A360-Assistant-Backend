...
# 무한정 커지는 것은 막는다. 닿으면 잘린 사실을 프롬프트·로그·진행 메시지 셋 다에 남긴다.
#
# 200 → 1000 (RPA-285). 200은 "실제 카탈로그는 보통 수십 개"라는 전제에서 나온 값인데,
# 제품 카탈로그를 통째로 주는 것이 실제 용례로 확인됐다(PAD 448개). 규칙 파서가 붙으면서
# 448개 추출이 LLM 0콜로 통과하므로, 이제 병목은 추출이 아니라 이 상한이었다.
#
# ⚠️ 진짜 비용은 개수가 아니라 **토큰**이다. 실측: 448개 메뉴 = 23,082토큰(200개는 10,416).
#    이 메뉴는 compose 호출마다 실리고 한 턴에 여러 번 돈다(실측 7~11콜). 즉 상한을 올린 만큼
#    턴 비용이 선형으로 는다.
#
# 한때 이 비용을 줄이려고 사용자 카탈로그용 검색기를 붙였다가 **걷어냈다.** 좁히면 비용은
# 줄지만 흐름도가 얇아진다 — 실측(PAD 448개, 같은 문서): 전량 액션 23~36개 vs 검색 8개.
# 사용자가 카탈로그를 준다는 것은 "이 어휘로 만들어라"는 뜻이고, 그중 무엇이 필요한지는
# 우리가 미리 좁힐 게 아니라 모델이 업무를 보고 고를 일이다(catalog_context 참조).
# 그래서 이 상한은 최적화 장치가 아니라 **병적 입력 방어**로만 남는다.
_MAX_USER_MENU_ACTIONS = 1000
