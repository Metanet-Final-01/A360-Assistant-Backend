...
class IntakeOutput(BaseModel):
    """v3 TaskPlan 출력 — 순서 있는 task 목록. route 단일 출력(구형)도 폴백 수용한다."""
...
def _guard_plan(tasks: list[str], state: TurnState) -> tuple[list[str], list[str]]:
    """LLM 계획에 결정론 가드를 적용한다 — v2의 단일 route 가드를 목록으로 일반화.

    반환: (정제된 plan, 가드 사유들). 하한 보장: 어떤 입력이 와도 plan은 최소 ["qa"].
    """
    notes: list[str] = []
    has_flow = bool((state.get("recommendation") or {}).get("steps"))

    # 미지 task 제거 + 중복 제거(순서 보존) + 상한
    seen: set[str] = set()
    plan: list[str] = []
    for t in tasks:
        if t not in ROUTES:
            notes.append(f"미지 task({t}) 제외")
            continue
        if t in seen:
            continue
        seen.add(t)
        plan.append(t)
    plan = plan[:_MAX_TASKS]

    # 흐름도 없는 edit → generate 강등 (수정 대상이 없으면 신규 산출)
    if ROUTE_EDIT in plan and not has_flow:
        plan = [ROUTE_GENERATE if t == ROUTE_EDIT else t for t in plan]
        # 강등으로 generate가 중복되면 하나만
        dedup: list[str] = []
        for t in plan:
            if t not in dedup:
                dedup.append(t)
...
def intake_node(state: TurnState) -> dict:
    """이번 턴의 TaskPlan을 판정한다 (v3). 계획은 LLM 1회, 이후 순회는 supervisor의 결정론.
    ...
    plan, notes = _guard_plan(tasks, state)
    if notes:
        reason = (reason + " / " if reason else "") + " · ".join(notes)

    # reason은 LLM이 만든 근거라 사용자 발화를 인용할 수 있어(PII 축적 방지) 길이를 제한해
    # 로깅한다. plan이 주 신호이고 reason은 보조 맥락이다.
    logger.info("intake plan=%s (%s)", plan, reason[:120])
    # 관측 전용(RPA-105): 최종 계획을 스트림에 흘린다 — message는 기존과 동일해 프론트
