...
def label_candidates(text: str, limit: int = 5) -> list[dict]:
    """사용자 문장에 등장한 한국어 라벨로 카탈로그 액션을 결정론 매칭한다 (v3 이중 언어 리졸버).

    "메시지 박스 넣어줘"류 이름 지목형 요청은 시맨틱 검색 전에 라벨 룩업이 정확하다 —
    KB의 액션 라벨(JAR ko_KR 로케일)이 한국어라 가능하다. 카탈로그가 순회를 지원하지
    않으면(테스트 스텁) 빈 목록. 반환: [{package, action, label}] (라벨 길이 내림차순 —
    더 구체적인 매칭 우선).
    """
    catalog = _make_catalog()
    iter_fn = getattr(catalog, "iter_action_schemas", None)
    if not callable(iter_fn) or not text:
        return []
    ...
    try:
        for s in iter_fn():
            ...
            if not isinstance(label, str) or len(label) < 2:
                continue
            if "".join(label.split()).lower() in compact_text:
                hits.append({"package": s.get("package"), "action": s.get("action"), "label": label})
    except Exception:  # noqa: BLE001 — 힌트 실패가 edit를 막지 않게
        return []
    hits.sort(key=lambda h: len(h.get("label") or ""), reverse=True)
    return hits[:limit]
