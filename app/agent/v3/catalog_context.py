...
def user_catalog_context(catalog: CatalogLookup, solution: str) -> CatalogContext:
    """사용자가 대화로 준 카탈로그 컨텍스트 — **검색 없이 전량이 메뉴**다.

    한때 큰 카탈로그를 인메모리 검색기로 좁히는 길을 두었다가 걷어냈다. 실측(PAD 448개,
    같은 업무정의서)이 좁히는 쪽에 불리했다:

        전량 448개 → 액션 23개 / 위반 32건(R3 29 = 필수값 미입력, **R1 환각 0**) / $0.0522
        검색  39개 → 액션  8개 / 위반 17건                                        / $0.0519

    위반 수만 보면 검색이 나아 보이지만 **흐름도가 얇다.** 전량 쪽 위반은 대부분 "이 값을
    채워라"라 질문 카드로 흡수되고, 액션이 환각 없이 3배 많다 — 실제로 굴러가는 흐름도는
    이쪽이다. 비용도 사실상 같았다(검색은 질의 확장 LLM 호출이 붙어 메뉴 절감분을 상쇄한다).

    사용자가 카탈로그를 준다는 것은 "이 어휘로 만들어라"는 뜻이다. 그중 무엇이 필요한지는
    검색이 아니라 **모델이 업무를 보고 고르는 것**이 맞다 — 우리가 미리 좁히면 고를 기회를
    뺏는다. 규모 방어는 `research._MAX_USER_MENU_ACTIONS`(메뉴 상한)가 맡는다.
    """
