...
async def open_pools() -> None:
    ...
    # 설정 누락은 연결 장애와 다르다. RAG 전용 DB가 없으면 서비스 DB로 저하하지 않고
    # lifespan을 실패시켜 잘못 배선된 인스턴스가 트래픽을 받지 못하게 한다.
    dsn = config.database_dsn()
    try:
        _pg_pool = AsyncConnectionPool(dsn, min_size=2, max_size=20, open=False)
        await _pg_pool.open()
