...
- action_reference: JAR이 없는 패키지의 리프 문서를 액션 후보로만 나열한 것
...
가장 단순한 대안이 `action_reference`(모든 리프를 필터링 없이 액션 후보로 취급)다.
...
def build_rag_documents(
    ...
    # 파라미터 스키마를 모르는 미검증 후보라 action_schema가 아니라 action_reference로
