...
## 한 번에 실행 (crawl → build → ingest)

리포 루트에서 `run_rag_pipeline.bat` 실행 → 옵션 선택:

- **옵션 1**: JAR 있는 패키지만 적재.
- **옵션 2**: 옵션 1 + JAR 없는 패키지 리프도 참고용(`action_candidate`)으로 같이 적재.

`packages.json`/`bots.jsonl`은 미리 준비돼 있어야 한다. 새 로직 없음 — 위 "사용 순서"의
기존 명령들을 그대로 순서대로 호출만 한다(`app/rag/scripts/`). 한 단계라도 실패하면 멈춘다.
...
- `action_candidate` — JAR 없는 패키지의 리프 문서를 필터링 없이 액션 후보로 나열한 것. 파라미터 스키마 없음, 추천 메뉴엔 안 뜨고 검색에만 쓰임.

## JAR 없는 패키지 (별도 파싱 Agent 담당 — 팀원 개발 예정)

Database/OCR/Python 등 일부 패키지는 JAR이 없어 `action_schema`가 비어 있다. 리프
문서가 "진짜 액션인지 참고/예제 문서인지"는 규칙으로도 문서 단위 LLM 판단으로도
못 푼다는 게 팀 결론(2026-07-10) — 그 판단 + 파라미터 스키마 추출은 별도 파싱 Agent가
맡는다. 여기서 이미 끝난 것(Agent가 다시 안 풀어도 됨): 패키지 판별, 패키지→카테고리→
리프 계층(사이트 메뉴 구조 그대로 따라감).

```bash
python -m app.rag.pipeline build-action-tree           # 구조만 저장: package_action_tree.json
python -m app.rag.pipeline export-for-agent             # Agent 입력 저장: agent_handoff.jsonl (구조화 HTML 포함)
python -m app.rag.pipeline export-naive-leaf-actions    # 참고용: 리프 전부를 액션 후보로 나열 (파라미터 없음)
```

- `agent_handoff.jsonl` 한 줄 = 리프 문서 1개: 패키지명/제목경로/제목/URL/구조화 HTML.
- 과거에 규칙/LLM으로 직접 풀려다 폐기한 시도: `_archive_pre_agent_handoff/README.md`
  (git 미포함, 다시 시도 전에 먼저 읽을 것).
