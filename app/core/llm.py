...
def cost_usd(
    ...
    - cached_tokens(입력 중 프롬프트 캐시 적중분)는 LLM_CACHED_INPUT_COST_PER_1M이 설정된
      ...
      캐시 단가 미설정이면 기존식(전액)으로 폴백 — 회귀 없음. 캐시 미반영이 계정 합산
      실측 1.21배 과대계상의 주원인이었고(캐시 입력은 정가의 10%), 적중률 높은 개별
      호출은 입력비가 최대 ~8.7배까지 부풀었다. (초기에 "4.7배"로 기록했으나 그건 두
      OpenAI 계정 중 하나만 본 오측정 — 기간 합계·계정 합산 재대사로 정정, RPA-273.)
...
def chat(
    ...
    *,
    ...
    ⚠ **temperature는 보내지 않는다.** 한때 계측 경로(분석·정형화·L2·L3)에 0을 걸어 재현성을
    확보했는데, 그건 그 인자를 받아 주는 모델에서만 성립하는 장치였다. 추론형 모델은 기본값
    외의 temperature를 거부하고(400), 그러면 호출마다 실패-재시도로 왕복이 두 배가 되면서
    재현성은 어차피 없다 — '있는 척'만 남는다. 그래서 인자 자체를 두지 않는다. 재현성이
    필요하면 샘플링이 아니라 **프롬프트와 후처리**로 좁힌다.
    """
