...
#   ① 아래 경계(4800 이상이어야): 단일 메시지의 진짜 상한은 관측값(3,400)이 아니라 **스키마
#      상한 `AgentTurnRequest.message`의 max_length=4000**이다(live 검증에서 정정 — 처음엔
#      관측값으로 잡았다). base(~772) + 4000 = **4,772**가 한 턴에 들어올 수 있는 최대치이므로,
#      LIMIT이 그 아래면 **1턴째부터 선행 가드(RPA-86)가 자동 compact를 오발동**한다 — 압축할
#      history가 없는데 compact 비용($0.00843/콜)만 나간다. 6000은 이 경계 위라 안전하다.
...
def _needs_auto_compact(gauge: dict, message: str) -> bool:
    ...
    ⚠️ RPA-86 원래 문구는 "**초대형 단일 입력**이 당턴을 넘치게 하는 갭을 닫는다"였으나, 그
    시나리오는 **스키마상 발생할 수 없다** (RPA-172 live 검증에서 확인): AgentTurnRequest.message가
    max_length=4000이라 한 턴 최대 입력은 base(~772)+4000 = 4,772 < LIMIT(6000)이다. 즉 **단일
    입력만으로는 임계를 넘길 수 없고**, 이 함수가 실제로 잡는 건 "**누적된 history + 큰 입력**"이다.
    (방어가 무의미한 건 아니다 — history가 쌓인 뒤 큰 입력이 오면 여기서 걸린다. 실측: 4턴째 발동.)

    LIMIT을 4,772 아래로 낮추면 그때는 단일 입력만으로 1턴째 오발동이 가능해진다 — 위
    _GAUGE_LIMIT_DEFAULT의 '아래 경계'가 그 이유다.
