...
#      현실 입력(한국어 산문)의 최악은 4,000자 × 1.08 ≈ **4,309토큰**이고, base(~772)를 더하면
#      **5,081**이다. 6000은 이 위라 여유 919를 남긴다 — 현실 입력으로는 초반 오발동이 없다.
#      (이모지 도배 같은 병리적 입력은 12,772까지 가능해 이 경계를 넘지만, 그건 **선행 가드가
...
def _estimate_message_tokens(text: str) -> int:
    ...
    (CodeRabbit #134), len은 한글과 거의 일치·영문(≈0.15 tok/char)엔 과대추정이다.

    ⚠️ 폴백이 **모든 입력에 안전한 건 아니다**: 이모지 3.0 / 희귀 CJK 2.0 tok/char라 len은
       2~3배 **과소**추정한다(RPA-172 실측). tiktoken이 깔려 있으면(기본) 무관하지만, 폴백으로
       떨어진 상태에서 이모지 도배가 오면 선행 가드가 스파이크를 놓친다. 폴백은 "없는 것보단
       낫다"지 상한 보장이 아니다.
    """
    if not text:
        return 0
...
def _needs_auto_compact(gauge: dict, message: str) -> bool:
    ...
    LIMIT을 현실 입력의 최악(base ~772 + 한글 4,000자 ≈ 4,309 = **5,081**) 아래로 낮추면 평범한
